...
** 防止 SQL 注入
   ...
   最好采用如下的形式:
   sql_query = "INSERT INTO table_name VALUES (%s, %s)"
   sql_values = (val0, val1)
   cursor.execute(sql_query, sql_values)

   注意，在后一种情况下，在 %s 周围不用加 ''，执行时会自动加上去，若加上了单引号，
   会报错。
   这时会产生一个问题，就是若使用如下形式时
   sql_query = "INSERT INTO %s VALUES (%s, %s)"
   即把表名也隐藏，这时也会对表名加上单引号，这在 sql 语句中是不对的，所以如果要
   想使用这种形式的 sql 语句，必须把表名显式添加上去。
** 在 sql 语句中使用 'LIKE'
...
** NOTE
   ...
   若不判断，则 cate 和 path 都会重复多次已经记录过的数据，需要加上的判断语句。* itertools
* itertools
** itertools.izip()
   注意如下的代码片段:
   ...
   将不会打印出 a 中的最后一个元素。如果 a 有多于 3 个的元素，将会打印出 a 中
   'a3' 之后的元素。我觉得可能的原因是，当 b 中的元素遍历完后，在 for 中又取出 a
   中的一个元素，但此时发现 b 中没有元素了，而 a 此时已经执行过 next() 函数，所
   以 a 中再取得元素时，将是 'a3' 之后的元素。
   
   解决方案是使用 itertools.izip_longest()
** itertools.groupby()
   它仅把连续相邻的 key 相同的项组合成 group，若 key 相同的项之间有其它不同 key
   的项，则只能把连续的项组成 group.
* 命令行运行 python
  通过
  $ python -c "python 语句"
...
** tuple 的 constructor
   ',' 是 tuple 的 construtor,而不是 '()'

* BEST PRACTICE
** 字符串处理
   在 python 脚本内部统一使用 unicode 处理文本,即读入文本时 decode 为 unicode 形
   式，写出时转换成指定的编码.
** 文本处理
   文本写入完后,立即通过 flush() 方法把数据从缓存中写入到文本.
