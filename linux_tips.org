...
** 虚拟系统支持 host-only 网络模式
   先检查 "偏好设置"-->"网络"-->"仅主机(Host-Only)网络" 中是否有虚拟网络，若没有，
   先添加.
   在安装系统前设置好采用 host-only 模式，然后安装系统.
   可参考 [[http://webees.me/installing-centos-6-4-in-virtualbox-and-setting-up-host-only-networking/][Installing CentOS 6.4 in VirtualBox and Setting up Host-only Networking]]
** clone 系统时的网络问题
   参考 [[https://www.centos.org/forums/viewtopic.php?t=7684][virtualbox centos6 guest - Device eth0 does not seem to be present]]
   可能会出现找不到网卡的问题.
   以 CentOS 为例:
   在 /etc/udev/rules.d/ 中找到网络配置，修改相应的 MAC.
   在 /etc/sysconfig/network-scripts/ 下修改相应的网卡.
   重启.

   还要注意修改下 HOSTNAME，在 /etc/sysconfig/network 中，修改后重启.
* 34、修改 Linux 环境变量
** 1、永久修改
...
** 常用
   ...
   显示窗口列表: C-a "
   重命名某个窗口: C-a A 
   给一个打开后的 screen 会话命名: C-a :sessionname 会话名
...
** 按时间排序
   ...
   # 按时间由前往最近排
   $ ls -l -t -v
* linux 下的 md5sum 值与 PHP 等计算的不同
  如 
  $ php -r "echo md5('flyer') . '\n'"
  与
  $ echo 'flyer' | md5sum
  的结果不同。
  这是因为 md5sum 计算时，其实计算的是 'flyer\n'，故需要按如下方式计算:
  $ echo -n 'flyer' | md5sum
* terminal 中向 stdout 中写入数据是否占用空间?
  不占用。

  + *stdout* 不是具体的文件或设备，它只是一个概念，即支持 write、flush 等的像文
    件一样的对象.
  + 除非是写入 *stdout* 中的数据进行重定向，否则向 *stdout* 中写入数据不会占用空
    间，因为 "Output to a console (or terminal emulator) is slow as it is,
    passing it through the file system would make it even slower, and may not
    allow the disks to spin down if there's a lot of output".
	重定向过程中，若是 OS 把该程序换出，则写入到 *stdout* 中需要重定向的数据会终
    止于 swap 空间.

  Ref:
  + [[http://stackoverflow.com/questions/18053647/does-writing-data-to-stdout-in-linux-occupy-disk-space][stackoverflow]]
* 禁止 beep 声
  有时通过 alsamixer 静音 Beep 没用，可在 ~/.profile 或 ~/.bashrc 中写入:
...
*** apr_socket_recv: Connection reset by peer (54)
	安装 apache-2.3.16 之上的版本或使用时加上 -r 参数.
* 查看端口占用
  可通过：
  $ losf -i:端口号
  查看对应的占用端口的应用的 pid.
...
* linux 中的 "线程" 和 "进程"
  ...
  Refer:
  + [[http://stackoverflow.com/questions/344203/maximum-number-of-threads-per-process-in-linux/344292#344292][Maximum number of threads per process in Linux?]]
* nginx
** php
*** 提示所有的 php 文件找不到
	检查两点:
	+ "include fastcgi_params;" 在所有的 "fastcgi_param *" 之前
    + "fastcgi_param" 中的 "SCRIPT_FILENAME" 是否正确 
* sentry
** Unable to reach Sentry log server: timed out (url: http://sentry.example.com:9100/api/2/store/)
   在 sentry 的配置文件中加入如下语句:

   SENTRY_SERVERS = ['http://0.0.0.0:9100/store/']
...
** 常用命令
   $ vagrant up  # 启动环境
   $ vagrant ssh  # 进入对应的虚拟机
* wordpress
  若配置时提示连接数据库失败，注意把数据库的 host 从 localhost 改为 '127.0.0.1'
* 无法 umount device
  参考 [[http://stackoverflow.com/questions/7878707/umount-a-busy-device][Umount a busy device]]
...
** 基础
   ...
   即 *find* 命令用来根据指定的参数查找 *文件* 或 *目录*.
** 常用命令例子
   $ find . -name data
   $ find /home/flyer -name data
   $ find /home/flyer -iname data
   $ find . -type d -name data
   $ find . -type f -name handle.py
   $ find . -type f -name "handle_*.py"
   ...
   $ find . -type f -name "handle.py" -exec rm -f {} \;
   $ find . -type f -name "*.txt" -exec rm -f {}\;
   $ find . -type f -empty
   $ find . -type f -name ".*"
   $ find . -user flyer -name handle.py
   $ find . -user flyer
   $ find . -group developer
   $ find . -user flyer -iname "*.txt"
   $ find . -mtime 50  # 找到前第 50 修改的数据
   $ find . -mtime +50  # 找到 50 天前修改的数据
   $ find . -mtime +50 -mtime -100  # 找到前 50~100 天之间被修改的数据
   $ find . -cmin -60  # 找到 60 分钟前被修改的数据
   $ find . -size 50M
   $ find . -size +50M -size -100M
   $ find . -size +100M -exec rm -rf {} \;
   $ find . -type f -name "*.mp3" -size +10M -exec rm -rf {} \;
   $ find . -maxdepth 2
   $ find . -maxdepth 3 -mindepth 5
   $ find . -maxdepth 1 -not -iname "handle_*.py"
   $ find . -inum 文件或目录的iNode号 -exec mv {} new-test-file-name \;
...
** 几个数字的表示
   ...
   + -N
     小于该值
** 深度
   指定的目录深度为 1，依次往后推其他深度。
** exec 后的几个符号
   -exec 后第一个参数为命令，之后的参数直到「;」 之前都是该命令的参数。
   参数中「{}」 都会用当前的文件名替换。
   为了防止参数和符号被 shell 转义，可用「\」 处理，如标志参数结尾的「；」，前面
   必须加上「\」 用来防止被 shell 转义。
* iftop
** what
...
** 常用操作
   ...
   按 q 退出监控
* tar 重命名文件
  $ tar xf TAR文件 -C 目标目录 --stripcomponents 1
* 格式化 json 文件
  可简单地使用 python 来实现:
  $ python3 -m json.tool JSON文件
* 查看已使用的 file handler 数
  $ lsof | wc -l
* ssh 无密码登录
  $ ssh-keygen -t rsa
...
* 修改 fd 数量
  ...
  永久修改 (对 linux)：
  修改 /etc/limits.conf 或 /etc/security/limits.conf (对不同的 linux 发行版文件
  位置可能不同)，添加类似如下的语句:
  ...
  * soft nofile 65535
  ...
  Ref:
  * [[http://stackoverflow.com/questions/34588/how-do-i-change-the-number-of-open-files-limit-in-linux][How do I change the number of open files limit in Linux?]]
  * [[https://easyengine.io/tutorials/linux/increase-open-files-limit/][Increase "Open Files Limit"]]
  * [[http://geekswing.com/geek/quickie-tutorial-ulimit-soft-limits-hard-limits-soft-stack-hard-stack/][Quickie tutorial: ulimit, soft limits, hard limits, soft stack, hard stack]]
