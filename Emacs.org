...
** 查看 log
   ...
   + 查看 log, 点 'l'
	 git log
** 解决冲突
   执行命令 **magit-ediff-resolve**.
   Use 'n'/'p' to move to the next/previous conflict, use 'a'/'b' to choose
   which changes (those in a A or B) should be the ones to keep in the merged
   files.
   You can always just switch to buffer C and edit what the merged version
   should like.
   Once you're done resolving all confilcts, just hit 'q' to exit the ediff
   merging session, ediff will prompt you to save the changes to the merged
   files. Save them and then move on to the next unmerged file.
* 查看所有可用的颜色
  通过如下命令:
